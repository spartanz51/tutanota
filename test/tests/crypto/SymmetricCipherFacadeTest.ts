...
import { InitializationVectorVariant, ParsedCiphertextAesCbc } from "../../../src/crypto/encryption/symmetric/decryption/ParsedCiphertext"
...
o.spec("SymmetricCipherFacadeTest", function () {
	...
	o.spec("Encrypt/decrypt bytes", function () {
		...
		o("encryptBytesDeprecatedCustomIv", function () {
			symmetricCipherFacade.encryptBytesDeprecatedCustomIv(aes256Key, plaintext, customInitializationVector)
		...
		o("encryptBytesDeprecatedUnauthenticatedCustomIv", function () {
			symmetricCipherFacade.encryptBytesDeprecatedUnauthenticatedCustomIv(aes256Key, plaintext, customInitializationVector)
			verify(
				aesCbcFacade.encrypt(
					aes256SubKeys,
					plaintext,
	...
	o.spec("Encrypt/Decrypt key", function () {
		...
		o("decryptKey 128", function () {
			...
			const ciphertext = concat(symmetricCipherVersionToUint8Array(parsedCiphertext.cipherVersion), parsedCiphertext.ciphertext)
		...
		o("decryptKeyDeprecatedUnauthenticatedFixedIv 256 no mac and no initialization vector (fixed) succeeds", function () {
			const parsedCiphertext: ParsedCiphertextAesCbc = {
				cipherVersion: SymmetricCipherVersion.UnusedReservedUnauthenticated,
				initializationVector: FIXED_INITIALIZATION_VECTOR,
				ciphertext: new Uint8Array([1, 2]),
			...
			const decryptedKey = symmetricCipherFacade.decryptKeyDeprecatedUnauthenticatedFixedIv(aes256Key, parsedCiphertext.ciphertext)
			o(decryptedKey).deepEquals(keyToEncrypt_256)
		})
	})
})
