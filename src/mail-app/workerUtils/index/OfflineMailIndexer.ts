...
import { assertWorkerOrNode, FULL_INDEXED_TIMESTAMP, GroupType, ImportStatus, NOTHING_INDEXED_TIMESTAMP, OperationType } from "@tutao/app-env"
...
import { Mail, MailBag, MailBox, MailDetailsBlob, MailDetailsBlobTypeRef, MailSetEntryTypeRef, MailTypeRef } from "../../../typerefs/entities/tutanota/TypeRefs"
import {
	elementIdPart,
	entityUpdateUtils,
	GENERATED_MIN_ID,
...
import { EntityAdapter, InstancePipeline, ModelMapper } from "@tutao/instance-pipeline"
...
// FIXME: This is very offline-specific. This code is using the persistence directly for now, as the indexed timestamp
//  stuff is not relevant.
...
export class OfflineMailIndexer implements MailIndexer {
	...
	async afterMailCreated(mailid: IdTuple): Promise<void> {
		const mail = await downloadNewMailData(mailid, this.entityClient, await this.mailFacade())
		if (mail != null) {
		...
		for (const event of events) {
			if (entityUpdateUtils.isUpdateForTypeRef(tutanotaTypeRefs.ImportMailStateTypeRef, event)) {
				...
				if (event.operation === OperationType.CREATE || event.operation === OperationType.UPDATE) {
					const importMailState = await this.entityClient.load(tutanotaTypeRefs.ImportMailStateTypeRef, [event.instanceListId, event.instanceId])
					...
					const importedMails = await this.entityClient.loadAll(tutanotaTypeRefs.ImportedMailTypeRef, importMailState.importedMails)
	...
	private async fullyIndexUser(user: User): Promise<void> {
		...
		if (!isEmpty(mailGroupsToAdd)) {
			...
			for (const group of mailGroupsToAdd) {
				...
				const mailboxGroupRoot = await this.entityClient.load(tutanotaTypeRefs.MailboxGroupRootTypeRef, group)
	...
	private async indexMailbox(mailbox: MailBox, mailboxProgress: (mailboxProgress: number, newMailsIndexed?: number) => Promise<unknown>) {
		...
		for (const mailbag of allMailBags) {
			await this.indexMailbag(mailbag, async (newMailsIndexed, currentMailbagMailsDownloaded) => {
				...
				//
				// FIXME: This is for testing/demonstration purposes. We should probably not do this...
	...
	private async loadNonRecentMail(
		...
		if (mail.mailDetailsDraft != null) {
			return await downloadNewMailData(mail._id, this.entityClient, await this.mailFacade())
		}
		...
		if (internalBlob == null) {
			...
			const blobFacade = await this.blobs()
		...
		const entityAdapter = await EntityAdapter.from(mailDetailsBlobTypeModel, blob, this.modelMapper)
		const mailDetailsUnmapped = await this.instancePipeline.cryptoMapper.decryptParsedInstance(
		...
		const attachments = await (await this.mailFacade()).loadAttachments(mail)
