...
class WizardDialog<T> implements Component<WizardDialogAttrs<T>> {
	...
	private renderBreadCrumbsWithStepTitle(
		...
		return m(
			...
			//16px;
			visiblePages.map((p, index) =>
				m(".flex.flex-column.flex-grow", [
					m(WizardPagingButton, {
						title: vnode.attrs.pages[index].attrs.stepTitle !== undefined ? vnode.attrs.pages[index].attrs.stepTitle : undefined,
						pageIndex: index,
						getSelectedPageIndex: () => selectedIndex,
						isClickable: () => a.allowedToVisitPage(index, selectedIndex),
						navigateBackHandler: (index) => a._goToPageAction(index),
...
export function createWizardDialog<T>({
	...
	dialogType: DialogType.EditLarge | DialogType.EditSmall | DialogType.ImapWizard
	cancelButtonText?: TranslationKey
	...
	switch (dialogType) {
		...
		case DialogType.ImapWizard: {
			wizardDialog = Dialog.openImapWizardDialog(wizardDialogAttrs.headerBarAttrs, () => m(child))
			break
		}
