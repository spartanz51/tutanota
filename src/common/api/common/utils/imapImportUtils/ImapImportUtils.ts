...
import { getConfigForDomain, ServerImapImportParams } from "./ImapKnownConfigs"

import { ImapAccount } from "../../../../desktop/imapimport/adsync/ImapSyncState"
...
export function importImapAccountToImapAccount(importImapAccount: tutanotaTypeRefs.ImportImapAccount): ImapAccount {
	const imapAccount = new ImapAccount(importImapAccount.host, parseInt(importImapAccount.port), importImapAccount.userName)
	imapAccount.password = importImapAccount.password ?? undefined
	const tokenEndpointResponse = importImapAccount.tokenEndpointResponse
	imapAccount.tokenEndpointResponse = tokenEndpointResponse !== null ? tutaDbTokenEndpointResponseToTokenEndpointresponse(tokenEndpointResponse) : undefined

	return imapAccount
}

export function tutaDbTokenEndpointResponseToTokenEndpointresponse(tokenEndpointResponse: tutanotaTypeRefs.TokenEndpointResponse): TokenEndpointResponse {
	return {
...
export function tokenEndpointResponseToTutadbTokenEndpointResponse(tokenEndpointResponse: TokenEndpointResponse): tutanotaTypeRefs.TokenEndpointResponse {
	return tutanotaTypeRefs.createTokenEndpointResponse({
