import { ArchiveDataType, MailMethod, MailPhishingStatus, MailState, ReplyType } from "@tutao/app-env"
import { RecipientList } from "../../../common/recipients/Recipient.js"
import { MailFacade, recipientToEncryptedMailAddress } from "./MailFacade.js"
...
export class ImportMailFacade {
	...
	private async _createAddedImportAttachments(
			...
			for (const file of files) {
				...
				attachments.push(
					tutanotaTypeRefs.createImportAttachment({
						...
						newAttachment: null,
						ownerFileKeyVersion: mailGroupKey.version.toString(), // FIXME
					}),
				)
			}
		...
		for (let index = 0; index < fileDataForUpload.length; index++) {
			...
			const importAttachment = draftAttachmentToImportAttachment(draftAttachment, file.original.fileHash, mailGroupKey, this.cryptoWrapper)
...
function draftAttachmentToImportAttachment(
	...
	const importAttachment = tutanotaTypeRefs.createImportAttachment({
	...
	const newFile = draftAttachment.newFile
	if (newFile != null) {
		...
		importAttachment.newAttachment = tutanotaTypeRefs.createNewImportAttachment({
			...
			// fixme why did we have a separate fileHashSessionKey in the first place? why not mimic the DraftAttachment/NewDraftAttachment structure?
			encFileHash: newFileHash ? cryptoWrapper.encryptString(fileHashSessionKey, newFileHash) : null,
