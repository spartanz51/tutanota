...
import { Country, CountryType, getByAbbreviation } from "../../../../app-env/CountryList"
...
export class XRechnungInvoiceGenerator {
	 ...
	 */
	generate(): Uint8Array {
		let taxCategory = this.getTaxCategory(this.invoice.vatType as VatType, getByAbbreviation(this.invoice.country)!)
		let stringTemplate =
	...
	private getTaxCategory(vatType: VatType, country: Country): TaxCategory {
		switch (vatType) {
			case VatType.NO_VAT: {
				return country.t === CountryType.EU ? TaxCategory.VAT_REVERSE_CHARGE : TaxCategory.OUTSIDE_SCOPE_OF_TAX
			}
	...
	private resolveBuyer(): string {
		...
		return XRechnungUBLTemplate.Buyer.replace("{buyerMail}", this.buyerMailAddress)
			.replace("{buyerStreetName}", urlEncodeHtmlTags(addressParts[1] ?? "STREET NAME UNKNOWN"))
			.replace("{buyerCityName}", urlEncodeHtmlTags(extractCityName(addressParts[2] ?? "")))
			.replace("{buyerPostalZone}", urlEncodeHtmlTags(extractPostalCode(addressParts[2] ?? "")))
			.replace("{buyerCountryCode}", urlEncodeHtmlTags(this.invoice.country))
			.replace("{buyerAddressLine}", urlEncodeHtmlTags(this.invoice.address.replaceAll("\n", " ")))
			.replace("{slotBuyerVatInfo}", this.resolveBuyerVatInfo())
			.replace("{buyerName}", urlEncodeHtmlTags(addressParts[0] ?? "BUYER NAME UNKNOWN"))
	}
...
<cbc:TaxExemptionReason>Steuerschuld des Leistungsempfängers (reverse charge)</cbc:TaxExemptionReason>`
	 ...
	 * @private
	 */
	private resolveInvoiceLine(invoiceItem: sysTypeRefs.InvoiceDataItem, taxCategory: TaxCategory, itemIndex: number): string {
	 ...
	 * @private
	 */
	private resolveCreditNoteLine(invoiceItem: sysTypeRefs.InvoiceDataItem, taxCategory: TaxCategory, itemIndex: number): string {
