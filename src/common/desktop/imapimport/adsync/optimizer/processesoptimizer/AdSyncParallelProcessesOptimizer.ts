...
export class AdSyncParallelProcessesOptimizer extends AdSyncProcessesOptimizer {
	...
	override optimize(): void {
		...
		const combinedAverageThroughputCurrent = this.getCombinedAverageThroughputInTimeInterval(currentInterval.fromTimeStamp, currentInterval.toTimeStamp)
		const combinedAverageThroughputLast = this.getCombinedAverageThroughputInTimeInterval(lastInterval.fromTimeStamp, lastInterval.toTimeStamp)

		// TODO remove logging
		console.log("(ParallelProcessOptimizer) Throughput stats: ... | " + combinedAverageThroughputLast + " | " + combinedAverageThroughputCurrent + " |")

		const lastUpdateAction = this.optimizerUpdateActionHistory.at(-1)
