...
export class AdSyncProcessesOptimizer extends AdSyncOptimizer implements AdSyncProcessesOptimizerEventListener {
	...
	protected startSyncSessionProcesses(amount: number) {
		const nextMailboxesToDownload = this.nextMailboxesToDownload(amount)
	...
	protected stopSyncSessionProcesses(amount: number) {
		const nextProcessIdsToDrop = this.nextProcessIdsToDrop(amount)

		for (const processId of nextProcessIdsToDrop) {
			const mailboxToDrop = this.runningProcessMap.get(processId)
			if (mailboxToDrop) {
				const timeToLiveIntervalMS = 1000 * (mailboxToDrop.syncSessionMailbox?.timeToLive ? mailboxToDrop.syncSessionMailbox?.timeToLive : 0) // conversion to milliseconds
				...
				if (mailboxToDrop.processStartTime + timeToLiveIntervalMS <= Date.now()) {
					if (mailboxToDrop.syncSessionMailbox) {
						const index = this.optimizedSyncSessionMailboxes.findIndex((mailbox) => {
	...
	protected nextProcessIdsToDrop(amount: number): number[] {
		...
		return [...this.runningProcessMap.entries()]
			...
			.sort(([_processIdA, valueA], [_processIdB, valueB]) => {
				const averageEfficiencyScoreA = valueA.syncSessionMailbox!.getAverageEfficiencyScoreInTimeInterval(
					currentInterval.fromTimeStamp,
					currentInterval.toTimeStamp,
				)
				const averageEfficiencyScoreB = valueB.syncSessionMailbox!.getAverageEfficiencyScoreInTimeInterval(
					currentInterval.fromTimeStamp,
					currentInterval.toTimeStamp,
				)
	...
	onDownloadUpdate(processId: number, syncSessionMailbox: ImapSyncSessionMailbox, downloadedQuota: number): void {
		const optimizerProcess = this.runningProcessMap.get(processId)
		if (optimizerProcess) {
	...
	onMailboxFinish(processId: number, syncSessionMailbox: ImapSyncSessionMailbox): void {
		...
		const mailboxIndex = this.optimizedSyncSessionMailboxes.findIndex((mailbox) => {
			return mailbox.mailboxState.path === syncSessionMailbox.mailboxState.path
		})
		if (mailboxIndex !== -1) {
			const isLastMailboxFinish = this.optimizedSyncSessionMailboxes.length === 1
