...
use crate::entities::generated::tutanota::{
	Mail, MailBox, MailDetailsBlob, MailSet, MailboxGroupRoot, SimpleMoveMailPostIn,
...
impl MailFacade {
	...
	/// Load and decrypt the `MailDetailsBlob` for the given `Mail`.
	///
	...
	pub async fn load_mail_details_blob(
		...
		let owner_key_version = mail._ownerKeyVersion.unwrap_or(0) as u64;
		...
		self.crypto_entity_client.decrypt_with_owner_key::<MailDetailsBlob>(
			parsed,
			&session_key,
			owner_enc_sk.clone(),
			owner_key_version,
