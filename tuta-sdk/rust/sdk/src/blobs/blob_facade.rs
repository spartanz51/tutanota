...
impl BlobFacade {
	...
	/// Load a blob element from a blob server by trying each server URL in order.
	/// On 401, the token is evicted and retried once.
	/// Mirrors TS `EntityRestClient.loadMultipleBlobElements()` + `doBlobRequestWithRetry()`.
	pub async fn load_blob_element(
				...
				match maybe_response {
					...
					Ok(RestResponse { status, .. }) => {
						match HttpError::from_http_response(status, &Default::default()) {
							Ok(HttpError::NotAuthorizedError) if attempt == 0 => {
		...
		Err(ApiCallError::InternalSdkError {
			error_message: "no blob servers responded successfully".to_owned(),
		})
	}
